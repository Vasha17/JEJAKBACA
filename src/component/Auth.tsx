...
// ── Helper: get/init guest join date from localStorage ────────────────────
export function getGuestMemberSince(): string {
  const key = "jejakbaca_guest_since";
  let stored = localStorage.getItem(key);
  if (!stored) {
    stored = new Date().toISOString();
    localStorage.setItem(key, stored);
  }
  return formatMemberSince(stored);
}
...
export function useAuth() {
  ...
  return { user, loading, signInWithGoogle, signOut, memberSince };
}
